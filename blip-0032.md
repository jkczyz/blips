 ...
 * MUST set the `domain_name` to the `domain_name` included in the `dnssec_query`-containing onion
   message being responded to.
 If the requested name does not exist (NXDOMAIN) or is not DNSSEC-signed:
   * SHOULD set `definitely_unresolvable`
 otherwise:
   * MUST NOT set `definitely_unresolvable`.
